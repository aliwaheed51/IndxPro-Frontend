...
export class SidebarMenuComponent implements OnInit {
	...
	async loadCurrentModule() {
		...
		await import('src/app/layout/user-management/user-management.module').then((result) => result.menuItems.length ? result.menuItems.map(item => this.currentModules.push(item)) : "");
		await import('src/app/layout/general-configuration/general-configuration.module').then((result) => result.menuItems.length ? result.menuItems.map(item => this.currentModules.push(item)) : "");
		await import('src/app/layout/hr/hr.module').then((result) => result.menuItems.length ? result.menuItems.map(item => this.currentModules.push(item)) : "")
		await import('src/app/layout/account/account.module').then((result) => result.menuItems.length ? result.menuItems.map(item => this.currentModules.push(item)) : "");
		await import('src/app/layout/DesignDirector/designdirector.module').then((result) => result.menuItems.length ? result.menuItems.map(item => this.currentModules.push(item)) : "");
		await import('src/app/layout/idmanager/idmanager.module').then((result) => result.menuItems.length ? result.menuItems.map(item => this.currentModules.push(item)) : "");
		await import('src/app/layout/control-items/control-items.module').then((result) => result.menuItems.length ? result.menuItems.map(item => this.currentModules.push(item)) : "");
	...
	async loadLazyModue(moduleToLoad: any) {
		...
		if (moduleToLoad.id === 'mnu_master') {
			...
			module = await import('src/app/layout/user-management/user-management.module');
		} else if (moduleToLoad.id === 'mnu_configuration') {
			module = await import('src/app/layout/general-configuration/general-configuration.module');
			...
			module = await import('src/app/layout/DesignDirector/designdirector.module');
			...
			module = await import('src/app/layout/control-items/control-items.module');
		}
	...
	loadItem(item: ComponentTabItem) {
		const exists = this.currentOpenItems.find((t) => t.componentId === item.componentId);
		if (exists) {
			exists.selected = true;
			this.currentOpenItems.filter((t) => t.componentId !== item.componentId).forEach((tab) => (tab.selected = false));
		} else {
